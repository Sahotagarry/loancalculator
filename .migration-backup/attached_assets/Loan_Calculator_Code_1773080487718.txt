...
import { calculateAmortization, Frequency } from "@/lib/amortization";
import {
  Card,
  CardContent,
  CardDescription,
      ...
      sections: [
        {
          ...
          children: [
                ...
                new DocxTableRow({
                  children: [
                    ...
                    new DocxTableCell({
                      children: [
                        new Paragraph({
                          text: formatCurrency(longTermPortion),
                          alignment: AlignmentType.RIGHT,
                          bold: true,
                        }),
                      ],
            ...
            new Paragraph({ text: "" }),
            new Paragraph({
              text: "The loan is secured by [Insert Description of Collateral/Security].",
              italics: true,
              color: "666666",
            }),
            new Paragraph({ text: "" }),
                ...
                <TableBody>
                  {result.schedule.map((row) => (
                    ...
                    >
                      ...
                      <TableCell
                        className={`text-right ${row.isInterestOnly ? "opacity-30" : "text-emerald-600 dark:text-emerald-400"}`}
                      >
                        {formatCurrency(row.principal)}
                      </TableCell>
                  ...
                  <TableRow className="bg-muted/50 font-bold border-t-2 sticky bottom-0 z-20 backdrop-blur-md">
                    ...
                    <TableCell className="text-right text-rose-700 dark:text-rose-500">
                      {formatCurrency(
                        result.schedule.reduce((sum, r) => sum + r.interest, 0),
                      )}
                    </TableCell>
